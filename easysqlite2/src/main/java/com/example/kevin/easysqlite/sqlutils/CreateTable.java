package com.example.kevin.easysqlite.sqlutils;
...
import com.example.kevin.easysqlite.annotation.NullSqlColumn;
...
import com.example.kevin.easysqlite.core.Debug;
...
public class CreateTable {
    ...
    public LinkedHashMap<String, String> getColumnNameAndType(Class<?> cls) {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            // more constraint column,one annotation use @xxxSqlColumn,other use @ConstraintColumnSql
                ...
                columnName = getName(f,annotation[1]);
    ...
    private String getConstraint(Annotation a) {
        ...
        if (a instanceof BiuniqueColumn) {
            constraint = "INTEGER";
        }
    ...
    private String getName(Field f, Annotation a) {
        String name = null;
        if (a instanceof BiuniqueColumn) {
            name = f.getType().getSimpleName() + "_id";
        }else{
            name = f.getName();
        }
        return name;
